...
Kontact::ProfileDialog::ProfileDialog( QWidget* parent, Qt::WindowFlags flags ) : KDialog( parent, flags )
{
    setModal( true );
    setCaption( i18n("Configure Profiles") );
    setButtons( KDialog::Ok|KDialog::Close );
    setButtonText( KDialog::Ok, i18n("Load Profile") );
    ...
    m_list = new K3ListView( mainWidget );
    ...
    connect( m_list, SIGNAL( itemRenamed( Q3ListViewItem*, const QString&, int ) ),
             this, SLOT( listItemRenamed( Q3ListViewItem*, const QString&, int ) ) );
    ...
    const QList<Kontact::Profile> profiles = Kontact::ProfileManager::self()->profiles();
...
void Kontact::ProfileDialog::slotButtonClicked(int button)
{
...
void Kontact::ProfileDialog::profileLoaded( const QString& id )
{
    ...
    if ( profile.isNull() )
        return;
    KMessageBox::information( this, i18n("The profile \"%1\" was successfully loaded. Some profile settings require a restart to get activated.", profile.name() ), i18n("Profile Loaded") );
}
...
void Kontact::ProfileDialog::saveToSelectedProfile()
{
    ...
    if ( profile.isNull() )
        return;
    if ( KMessageBox::Yes != KMessageBox::warningYesNo( this, i18n("The profile \"%1\" will be overwritten with the current settings. Are you sure?", profile.name() ), i18n("Save to Profile"), KStandardGuiItem::overwrite(), KStandardGuiItem::cancel() ) )
        return;
...
void Kontact::ProfileDialog::deleteSelectedProfile()
{
    ...
    if ( profile.isNull() )
        return;
    if ( KMessageBox::Yes != KMessageBox::warningYesNo( this, i18n("Do you really want to delete the profile \"%1\"? All profile settings will be lost!", profile.name() ), i18n("Delete Profile"), KStandardGuiItem::del(), KStandardGuiItem::cancel() ) )
        return;
...
void Kontact::ProfileDialog::exportSelectedProfile()
{
    ...
    {
        KMessageBox::information( this, i18n("The profile \"%1\" was successfully exported.", profile.name() ), i18n("Profile Exported") );
    }
    else
    {
        // TODO print error
...
void Kontact::ProfileDialog::profileAdded( const QString& id )
{
    ...
    Q3ListViewItem* const item = new Q3ListViewItem( m_list );
...
{
    Q3ListViewItem* item = m_profileToItem[id];
    Q_ASSERT( item );
...
void Kontact::ProfileDialog::listItemRenamed( Q3ListViewItem* item, const QString& text, int col )
{
...
void Kontact::ProfileDialog::updateButtonState()
{
    ...
    button( KDialog::Ok )->setEnabled( hasSelection );
